...
def classify_report(title, reference_period_raw=None):
...
def parse_archive_page(html_text, page_no):
    ...
    for href, block in pattern.findall(html_text):
        ...
        sub_category, period_type, year, month, quarter, months_covered, start_date, end_date = classify_report(title, reference_period_raw)
...
def parse_report_attachments(report):
    ...
    for href, inner in links:
        ...
        role = attachment_role(aurl, label)
